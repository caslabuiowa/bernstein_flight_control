...
# -*- coding: utf-8 -*-
"""
Created on Thu Jun 20 13:21:29 2019
...
NUM_VEH = 3
MAX_SPEED = 1 # m/s
FINAL_PTS1 = [   # Ending points in meters
            (2, 1),
            (-3.5, 1),
            (-0.3, 1)
            ]
FINAL_PTS2 = [   # Ending points in meters
            (-3.5, -1),
            (-0.75, -1),
            (1, -1)
            ]
FINAL_PTS3 = [   # Ending points in meters
            (-3.5, 1),
            (2, 1),
            (-0.3, 1)
            ]
FINAL_PTS4 = [   # Ending points in meters
            (-0.75, -1),
            (-3.5, -1),
            (1, -1)
            ]
FINAL_PTS5 = [   # Ending points in meters
            (2, 1),
            (-0.3, 1),
            (-3.5, 1)
            ]
FINAL_PTS6 = [   # Ending points in meters
            (-0.75, -1),
            (1, -1),
            (-3.5, -1)
            ]

FINAL_PTS_LIST = [FINAL_PTS1, FINAL_PTS2, FINAL_PTS3, FINAL_PTS4, FINAL_PTS5,
                  FINAL_PTS6]
...
#                     lambda x: pose_callback(x, 'AR_2', pose_dict))
    ...
    # =========================================================================
    for trajectoryIter, FINAL_PTS in enumerate(FINAL_PTS_LIST):
...
#        for i in range(NUM_VEH):
#            for idx in range(trajectories[i].shape[1]):
#                pose.pose.position.x = trajectories[i][0, idx]
#                pose.pose.position.y = trajectories[i][1, idx]
...
#                print(curveIdx)
                ...
                for i in range(NUM_VEH):
                    ...
                    path.header.stamp = cur_time

                    path.poses.append(cmd_pose)

                    pubListPose[i].publish(cmd_pose)
                    pubListTwist[i].publish(cmd_twist)
                    pubListPath[i].publish(path)
                curveIdx += 1
            if curveIdx > 100:
                break

        np.save('SavedTrajectory_{}'.format(trajectoryIter), trajectories)
